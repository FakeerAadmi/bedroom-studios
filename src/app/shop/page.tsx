...
export default function ShopPage() {
  ...
  const { liveCategories, isStoreLoading } = useStoreState();
  ...
  const filteredCategories = useMemo(() => {
    if (!liveCategories || liveCategories.length === 0) return [];
    
    const searched = liveCategories
      .map((category) => ({
        ...category,
    ...
    return searched;
  }, [dropFilter, materialFilter, search, sort, liveCategories]);

  if (isStoreLoading) {
    return (
      <PageShell className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-16">
        <div className="flex justify-center py-20 text-ink/50 font-mono animate-pulse text-sm uppercase tracking-widest">
          Loading Shop...
        </div>
      </PageShell>
    );
